...
>
> L'utilisation de BlackOps sur des systèmes, réseaux ou serveurs dont vous n'êtes **PAS LE PROPRIÉTAIRE** ou sans **AUTORISATION EXPLICITE** est :
>
...
>
> ⚠️ CE LOGICIEL EST FOURNI "EN L'ÉTAT", SANS GARANTIE D'AUCUNE SORTE. L'AUTEUR DÉCLINE TOUTE RESPONSABILITÉ EN CAS D'USAGE MALVEILLANT OU NON AUTORISÉ.
...
6. [Modules disponibles](#6-modules-disponibles)
...
12. [Commandes rapides (cheat sheet)](#12-commandes-rapides-cheat-sheet)
13. [Support & informations](#13-support--informations)
...
| Suffixe           | Usage                        |
|-------------------|------------------------------|
| `_scrambled.lst`  | `bops.py` (scan multi-services) |
| `_27017.lst`      | MongoDB                      |
| `_3306.lst`       | MySQL                        |
| `_5432.lst`       | PostgreSQL                   |
| `_6379.lst`       | Redis                        |
| `_9200.lst`       | Elasticsearch                |
| `_6443.lst`       | Kubernetes                   |
...
> **Attention :** ces scripts utilisent des fichiers d'entrée fixes (`_*.lst`) et ne supportent pas les options avancées de la CLI.
...
- Bases de données exposées (`listDatabases`)
...
> ⚠️ Ne jamais committer `data/output/` ou `logs/` dans Git

Les tokens Kubernetes dans `config.yaml` doivent rester secrets. Utilisez `.gitignore` pour exclure :
...
Pour 1000 IPs avec 6 modules chacun (6 000 endpoints) :

| Critère        | Valeur estimée              |
|----------------|-----------------------------|
| Temps          | 2 à 4 heures (avec `--deep`)|
| Mémoire        | ~500 Mo                     |
